            ...
            if ($decode_clone_tags != '' && $decode_clone_tags != NULL) {
                ...
                foreach ($decode_clone_tags as  $create_tags_name_page) {                
                    ...
                    foreach ($create_tags_name_page as $sagkey => $sagvalue) {
                        ...
                        $replace_content = str_ireplace('{{' .$sagkey . '}}', $sagvalue, $replace_content);
                        $all_meta_content = str_ireplace('{{' . $sagkey . '}}', $sagvalue, $all_meta_content);
                    }
                    $titleContainsTag = 0;
                    ...
                    foreach ($create_tags_name_page as $titlekey => $titlevalue) {
                        $replace_title = str_ireplace('{{' . $titlekey . '}}', $titlevalue, $replace_title);
                        $all_meta_title = str_ireplace('{{' . $titlekey . '}}', $titlevalue, $all_meta_title);
                    }
                    foreach ($create_tags_name_page as $imagekey => $imagevalue) {
                        $image_alt = str_ireplace('{{' . $imagekey . '}}', $imagevalue, $image_alt);
                        $new_image_img = str_ireplace('{{' . $imagekey . '}}', $imagevalue, $new_image_img);
                        $new_image_Description = str_ireplace('{{' . $imagekey . '}}', $imagevalue, $new_image_Description);
                    }


                    foreach ($create_tags_name_page as $permalinkkey => $permalinkvalue) {
                        $new_post_permalink = str_ireplace('1t1' . $permalinkkey . '1t1',' '.$permalinkvalue.' ', $new_post_permalink);  
                    }                    
